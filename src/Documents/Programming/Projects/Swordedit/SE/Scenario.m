...
- (void)createVehicleReference:(TAG_REFERENCE)tag
{
    vehicle_reference *newtmpRefPointer, tmpRef;
    ...
    vehi_ref_count++;
    header.VehicleRef.newChunkCount = vehi_ref_count;
    //NSLog(@"CR1");
    newtmpRefPointer = (vehicle_reference *)malloc(sizeof(vehicle_reference) * (vehi_ref_count+1));
    //NSLog(@"CR2");
    /// Copy the old stuff
    memcpy(newtmpRefPointer, vehi_references, (sizeof(vehicle_reference) * (vehi_ref_count - 1)));
    //NSLog(@"CR3");
    
    memcpy(&newtmpRefPointer[vehi_ref_count-1], &tmpRef, sizeof(vehicle_reference));
    //NSLog(@"CR4");
    // Destroy the old data
    ...
    //NSLog(@"CR5");
    // Set the pointer
    vehi_references = newtmpRefPointer;
    //NSLog(@"CR6");
...
- (unsigned int)createVehicle:(float *)coord
{
    ...
    NSLog(@"Spawns: %d", vehicle_spawn_count);
	return (vehicle_spawn_count-1);
}
	...
	*hack;
	...
	for (i = 0; i < 79; i++)
	{
		...
		if ((tmpReflex->offset > 0) && (tmpReflex->offset < [self tagLength]) && (tmpReflex->offset != negativeOffset))
			...
			realReflexives++;
        else if (tmpReflex->refNumber==7 && vehicle_spawn_count)
            realReflexives++;
        else if (tmpReflex->refNumber==8 && vehi_ref_count)
        realReflexives++;
	}
	...
	x = 0;
     ...
     NSLog(@"VEHICLE REFS %d", vehi_ref_count);
	for (i = 0; i < 79; i++)
	{
		tmpReflex = (reflexive *)_reflexLookup[i];
        ...
        else if (tmpReflex->refNumber==7 && vehicle_spawn_count)
		{
            if (!singlePlayer)
            {
                ...
                tmpReflex->newChunkCount=vehicle_spawn_count;
                tmpReflex->chunkSize=VEHICLE_SPAWN_CHUNK;
                
                reflexive *scenery = (reflexive *)_reflexLookup[largest_index];
                
                int increment_offset = scenery->newChunkCount * scenery->chunkSize;
                if (vehicle_spawn_count)
				increment_offset+=vehicle_spawn_count*VEHICLE_SPAWN_CHUNK;
                
                int increment_offset2 = vehicle_spawn_count*VEHICLE_SPAWN_CHUNK;
                ...
                _reflexLookup[7]=(reflexive *)tmpReflex;
		...
		{
            NSLog(@"VEHICLEREFS %d", vehi_ref_count);
            if (!singlePlayer)
            {
                //Find the last offset
                int largest_offset = 0;
                ...
                tmpReflex->newChunkCount=vehi_ref_count;
                tmpReflex->chunkSize=VEHICLE_SPAWN_CHUNK;
                ...
                int increment_offset = scenery->newChunkCount * scenery->chunkSize;
                if (vehi_ref_count)
				increment_offset+=vehi_ref_count*VEHICLE_SPAWN_CHUNK;
                
                int increment_offset2 = vehi_ref_count*VEHICLE_SPAWN_CHUNK;
                tmpReflex->offset= scenery->offset + increment_offset;
                ...
                _reflexLookup[8]=(reflexive *)tmpReflex;
