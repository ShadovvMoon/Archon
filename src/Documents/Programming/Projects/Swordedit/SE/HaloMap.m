...
- (int)loadMap
{
	...
	NSMutableArray *vehi_offsets = [[NSMutableArray alloc] init];
	...
	for (i = 0; i < indexHead.tagcount; i++)
	{
		...
		else
		{
			if (memcmp([tempTag tagClassHigh], (isPPC ? "vehi" : "ihev"), 4) == 0)
			{
                ...
                [vehi_offsets addObject:tempTag];
                //[self seekToAddress:nextOffset];
	...
	*/
	...
	for (i = 0; i < indexHead.tagcount; i++)
	{
        ...
        else if (memcmp([tempTag tagClassHigh], (isPPC ? "vehi" : "ihev"), 4) == 0)
		{
			vehi_counter++;
		}
		else if (memcmp([tempTag tagClassHigh], (isPPC ? "bitm" : "mtib"), 4) == 0)
    ...
    if ([mapScenario vehi_ref_count] < vehi_counter)
	{
		int response = NSRunAlertPanel(@"Vehicles detected", @"Swordedit has detected vehicle tags which are not referenced in the scenario. Would you like to rebuild references?", @"No", @"OK", nil);
		if (response != NSOKButton)
		{
			NSLog(@"CREATING VEHICLE REFERENCES");
			//CREATE THE MACHINE REFERENCES
			NSLog(@"Total vehicles... %d", [vehi_offsets count]);
			for (i=0; i<[vehi_offsets count];i++)
			{
				MapTag *tag = [vehi_offsets objectAtIndex:i];
                NSLog([tag stringTagClassHigh]);
				...
				[mapScenario createVehicleReference:machine];
			}
