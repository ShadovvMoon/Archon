...
void CRender::RenderTextureAndLightmaps(UINT mesh_index)
{
  ...
  pMesh = gBspManager.GetActiveBspXboxSubmesh(mesh_index);
  ...
  {
    //gShaderManager.ActivateShader(pMesh->RenderTextureIndex);
    gShaderManager.ActivateLightmapBlend(pMesh->RenderTextureIndex, 0, pMesh->LightmapIndex);
...
void CRender::RenderPcSubmeshLines(UINT mesh_index)
{
  ...
  pMesh = gBspManager.GetActiveBspPcSubmesh(mesh_index);
...
void CRender::RenderVisibleBsp(BOOL bSelectMode)
{
  ...
  if(m_bEnableRender)
  {
    ...
    {
      pMesh = gBspManager.GetActiveBspPcSubmesh(i);
...
void CRender::RenderXboxMap(BOOL bSelectMode)
{
  ...
  if(m_bEnableRender)
  {
    ...
    for(int i=0; i<mesh_count; i++)
    {
      pMesh = gBspManager.GetActiveBspXboxSubmesh(i);
