...
void CBspManager::LoadVisibleBspInfo(REFLEXIVE BspChunk, UINT version)
{
  ...
  if(BspChunk.Count > 0)
  {
    ...
    for(int i=0; i<BspChunk.Count; i++)
    {
      ...
      temp = CheckForReference(m_pMapFile, m_pBspInfo[i].NamePtr, m_Magic);
      m_BspNames.Add(temp);
...
SUBMESH_INFO* CBspManager::GetActiveBspPcSubmesh(UINT mesh_index)
{
...
SUBMESH_INFO* CBspManager::GetActiveBspXboxSubmesh(UINT mesh_index)
{
  SUBMESH_INFO *pMesh = NULL;
